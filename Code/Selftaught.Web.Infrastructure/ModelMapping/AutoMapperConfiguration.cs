namespace Selftaught.Web.Infrastructure.ModelMapping
{
    ...
    public class AutoMapperConfiguration
    {
        ...
        public AutoMapperConfiguration(Assembly assembly)
        {
        ...
        private static void LoadCustomMappings(IEnumerable<Type> types)
        {
            var maps = (from t in types
                        from i in t.GetInterfaces()
                        where typeof(IRequiresCustomMapping).IsAssignableFrom(t) &&
                              !t.IsAbstract &&
                              !t.IsInterface
                        select (IRequiresCustomMapping)Activator.CreateInstance(t));
